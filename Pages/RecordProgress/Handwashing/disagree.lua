...
local function takeAnswer(event)
	if event.phase == "ended" then
		if event.target.id == "Yes" then
			...
			composer.gotoScene("Pages.RecordProgress.Handwashing.tut1",{effect="slideLeft"})
		elseif event.target.id == "No" then 
			print("user answered no")
			composer.gotoScene("Pages.RecordProgress.Handwashing.disagree2",{effect="slideLeft"})
...
function scene:show( event )
	...
	local questionText = display.newText( "You might need to practice the correct correct way to wash your hands.\n\nWould you like to see the tutorial again?", CentreX, CentreY, ScreenWidth - 25, 0,native.systemFont, 26 )
