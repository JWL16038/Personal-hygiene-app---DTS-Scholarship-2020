...
local function takeAnswer(event)
	if event.phase == "ended" then
		if event.target.id == "Yes" then
			...
			composer.gotoScene("Pages.RecordProgress.Facemask.tut1",{effect="slideLeft"})
		elseif event.target.id == "No" then 
			print("user answered no")
			composer.gotoScene("Pages.RecordProgress.Facemask.disagree2",{effect="slideLeft"})
...
function scene:show( event )
	...
	local questionText = display.newText( "Would you like to see the tutorial again?", CentreX, CentreY, ScreenWidth - 25, 0,native.systemFont, 26 )
