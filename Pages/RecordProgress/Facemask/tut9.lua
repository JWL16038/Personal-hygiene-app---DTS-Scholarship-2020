...
local function gotoNext(event)
	if event.phase == "ended" then
		composer.gotoScene("Pages.RecordProgress.Facemask.tut10",{effect="slideLeft"})
	end
end
...
function scene:show( event )
	...
	local questionText = display.newText( "Removing the face mask:\n\nClean your hands with soap and water or hand sanitizer before touching the mask.Avoid touching the front of the mask. The front of the mask is contaminated. Only touch the ear loops/ties/band.Follow the instructions below for the type of mask you are using.", CentreX, CentreY, ScreenWidth - 25, 0,native.systemFont, 26 )
