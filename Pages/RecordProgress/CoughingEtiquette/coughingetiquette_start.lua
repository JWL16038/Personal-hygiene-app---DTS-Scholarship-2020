...
local function takeAnswer(event)
	if event.phase == "ended" then
		if event.target.id == "Yes" then
			...
			composer.gotoScene("Pages.RecordProgress.CoughingEtiquette.agree",{effect="slideLeft"})
		elseif event.target.id == "Maybe" then 
			print("user answered maybe")
			composer.gotoScene("Pages.RecordProgress.CoughingEtiquette.disagree",{effect="slideLeft"})
...
function scene:show( event )
	...
	local questionText = display.newText( "Did you cover your mouth when you cough or sneeze today?", CentreX, CentreY, ScreenWidth - 25, 0,native.systemFont, 26 )
